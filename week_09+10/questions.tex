...
\question\TFQuestion{T}{Given the choice between (A) having indexes on an SSD and tables on an HDD, and (B) having tables on an HDD and indexes on an SDD, option (A) will generally be faster.}
...
CREATE TABLE tweets AS (
	...
	id_user BIGINT REFERENCES users(id_users),
